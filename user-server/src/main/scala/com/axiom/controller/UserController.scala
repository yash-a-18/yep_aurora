  ...
  def apply(): Http[UserService, Throwable, Request, Response] =
    Http.collectZIO[Request] {
      case req @ (Method.GET -> Root / "user" / "getall") =>
        ...
        UserService.getUsers().map(out  => Response.json(out.toJson))

      case req @ (Method.GET -> Root / "user" / "sortByFirstName") =>
        UserService.getSortByFirstName().map(out => Response.json(out.toJson))
